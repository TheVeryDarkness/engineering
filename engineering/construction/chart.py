...
def __write_vertical_table(
    ...
    """
    data: sections * processes
    """
    m = len(data)
    for i, section in enumerate(data):
        for j, t in enumerate(section):
            length = time_cost[j]
            file.write(
                f'<polyline points="{X0+t*__X},{Y0+(m-i)*__Y} {X0+(t+length)*__X},{Y0+(m-i-1)*__Y}" style="{__LINE_STYLE}"/>            '
            )


def __write_multiline_text(TEXT: str, X: int, Y: int, W: int, H: int, file: TextIO):
    ...
    for i_r in range(0, ROWS):
        ...
        # 施工段名称（分行）
        file.write(
            f'<text textLength="{__FONT_SIZE*len(S)}" x="{X}" y="{Y+(i_r-ROWS/2+1)*__FONT_SIZE}" {__TEXT_PROP}>{S}</text>'
        )
...
"""
    ...
    if vertical:  # 垂直图表
        ...
        file.write(
            f"""
<!--Description-->
<text x="{((1+W)/2)*__X}" y="{__Y-__MARGIN}" {__TEXT_PROP}>施工进度/{UNIT}</text>
            """
        )
...
<polyline points="{0},{i*__Y} {WIDTH},{i*__Y}" style="{__LINE_STYLE}"/>
    """
        ...
        file.write("<!--Process-->")
        __write_vertical_table(
            __X,
            __Y * 2,
            Ns,
            [[START[j] + i for j in range(n)] for i in range(m)],
            file,
        )
        ...
        for t in range(1, W):
            file.write(
                f'\n<text x="{t*__X+__X/2}" y="{__Y*2-__MARGIN}" {__TEXT_PROP}>{t*K}</text>'
            )
        ...
        file.write(
            f"""
<!--Description-->
<text x="{((2+W)/2)*__X}" y="{__Y-__MARGIN}" {__TEXT_PROP}>施工进度/{UNIT}</text>
            """
        )
...
<polyline points="{__X*1 if i not in[1,2] else 0 if i in [2] else __X*2},{i*__Y} {WIDTH},{i*__Y}" style="{__LINE_STYLE}"/>
            ...
            if j >= 0:
                if j < TI:
                    # 施工队名称
                    file.write(
                        f'<text x="{(3/2)*__X}" y="{i*__Y-__MARGIN}" {__TEXT_PROP}>{i_section}-{j}</text>'
                    )
                for _j in range(0, K):
...
<polyline points="{__X*(2+j+BEGIN+_j*NI)},{i*__Y+Y_OFFSET} {__X*(2+j+BEGIN+(_j+1)*NI)},{i*__Y+Y_OFFSET}" style="{__LINE_STYLE}"/>
        ...
        for t in range(2, W):
            # 时间
            file.write(
                f'\n<text x="{t*__X+__X/2}" y="{__Y*2-__MARGIN}" {__TEXT_PROP}>{(t-1)*K}</text>'
            )
